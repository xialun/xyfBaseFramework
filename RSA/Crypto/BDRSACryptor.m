                ...
                error:(BDError *)error
{
    ...
    for (int i=0; i<blockCount; i++) {
        ...
        OSStatus secStatus = SecKeyEncrypt(publicKey, kSecPaddingPKCS1, (const uint8_t *)[buffer bytes], [buffer length], cipherBuffer, &cipherBufferSize);
                ...
                error:(BDError *)error
{
    ...
    NSData *data = [cipherText base64DecodedData];
    ...
    for (int i = 0; i < blockCount; i++) {
        ...
        size_t plainLen = SecKeyGetBlockSize(privateKey);
        void *plain = malloc(plainLen);
        
        OSStatus status = SecKeyDecrypt(privateKey, kSecPaddingPKCS1, cipher, cipherLen, plain, &plainLen);
